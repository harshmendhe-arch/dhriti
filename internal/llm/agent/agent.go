...
import (
	...
	"github.com/opencode-ai/dhriti/internal/llm/prompt"
...
func createAgentProvider(agentName config.AgentName) (provider.Provider, error) {
	...
	if cfg.GatewayEnabled() {
		gwKey := cfg.Gateway.APIKey
		if gwKey == "" {
			// Login stores OpenAI key under providers.openai — reuse as Bearer.
			gwKey = config.GatewayBearerToken()
		}
		opts = append(opts, provider.WithGateway(cfg.Gateway.URL, gwKey, cfg.Gateway.Model))
	}
	...
	if cfg.GatewayEnabled() {
		// All gateway traffic speaks OpenAI Realtime regardless of model catalog provider.
