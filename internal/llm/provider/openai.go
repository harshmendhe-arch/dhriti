...
func (o *openaiClient) send(ctx context.Context, messages []message.Message, tools []tools.BaseTool) (response *ProviderResponse, err error) {
	// Gateway mode: one-shot Realtime session over WebSocket (no local API key).
	if o.providerOptions.gateway.enabled() {
		return o.sendViaGateway(ctx, messages, tools)
	}
...
func (o *openaiClient) shouldRetry(attempts int, err error) (bool, int64, error) {
	// WebSocket errors are plain errors; check for rate-limit signals in the message.
	errMsg := err.Error()
	if contains(errMsg, "rate limit", "rate_limit", "quota exceeded", "too many requests") {
	...
	// HTTP API errors (from send()) still use the typed openai.Error.
